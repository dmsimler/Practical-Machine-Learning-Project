...
modelResultsPhase0 <- CreateModelPerfResultsDatasetsWWithinPhaseComparison(list(phase0RFFit, phase0XGBoostFit,
                                                                                phase0BaggedTreeFit,
                                                                                phase0NaiveBayesFit, 
                                                                                phase0LDAFit,
                                                                                phase0MultinomialFit,
                                                                                phase0RpartFit, phase0SVMFit),
...
$m = 8! / (8 - 2)! = 56$  
using the standard $\alpha = 0.05$, the significance level $= 0.05 / 56 = 0.00089$

$H_{0}$ will therefore be rejected if for a given model-to-model comparison t-test, the corresponding *p-value* is less than 0.00089. In that case, the data provides convincing evidence that Model 1's error rate is actually lower than Model 2's (i.e. $H_{a}$ is true).
...
# -------------------------------------------------------------------------------------------------------
ModelComparisonOrderedLevelsPhase1 <- c("multinom1 - multinom0",  "lda1 - lda0", "svmRadial1 - svmRadial0", 
                                        "treebag1 - treebag0", "xgbTree1 - xgbTree0", "rf1 - rf0", 
                                        "rpart1 - rpart0", "nb1 - nb0",
                                        "nb0 - nb1", "rpart0 - rpart1", "rf0 - rf1", "xgbTree0 - xgbTree1", 
                                        "treebag0 - treebag1",  "svmRadial0 - svmRadial1", "lda0 - lda1", 
                                        "multinom0 - multinom1") 
...
Again keeping the *Bonferroni correction* in mind, the significance values used for each of the t-tests will be different this time, i.e. the significance level $= 0.05 / 16 = 0.00313$.  16 is the total number of model comparisons calculated as $m_{Total} = n * m_{ComparisonsPerModel}$ where $n = 8$ is the number of models each with $m_{ComparisonsPerModel} = 2$ comparisons: Phase 1 Mean CV Error < Phase 0 Mean CV Error, and Phase 0 Mean CV Error < Phase 1 Mean CV Error for the model in question.
...
* *In this phase, the classifier with the lowest error rate was the Random Forest (as opposed to Phase 0 where it was the Gradient Boosted Tree).  The classifier with the highest error rate was again the RPart decision tree (althought its error rate was slightly lower this time).* 
...
* *Despite the statistically significant decrease in performance, the four classifiers with the lowest CV errors (rf, xgbTree, treeBag, and svmRadial) still generalized very well to the small test set with 0% error as in Phase 0. The Multinomial Logistic Regression (multinom) and RPart Decision Tree (rpart) classifiers saw their error rates reduced by 5% (to .35 and .55 respectively). Only the LDA classifier showed worse performance (error rate up 10% from .3 to .4)* 
...
ModelComparisonOrderedLevelsPhase2 <- c("xgbTree2 - xgbTree0", "rf2 - rf0", "treebag2 - treebag0", 
                                        ...
                                        "multinom2 - multinom0", "lda2 - lda0", "rpart2 - rpart0",
                                        "xgbTree0 - xgbTree2", "rf0 - rf2", "treebag0 - treebag2",
                                        "svmRadial0 - svmRadial2", "nb0 - nb2", 
                                        "multinom0 - multinom2", "lda0 - lda2", "rpart0 - rpart2") 
...
Again keeping the *Bonferroni correction* in mind, the significance values used for each of the t-tests will be different this time, i.e. the significance level $= 0.05 / 16 = 0.00313$.  16 is the total number of model comparisons calculated as $m_{Total} = n * m_{ComparisonsPerModel}$ where $n = 8$ is the number of models each with $m_{ComparisonsPerModel} = 2$ comparisons: Phase 2 Mean CV Error < Phase 0 Mean CV Error, and Phase 0 Mean CV Error < Phase 2 Mean CV Error for the model in question.
...
```{r, echo=FALSE, fig.path="figures/", fig.align='center', fig.cap='Figure 9: Heat map of predictions on the test set with five classes for each model type. The true classes are shown in the row labels while columns quantify the probabilities for each class (labeled A through E)', fig.height=14, fig.width=14, fig.show='asis', warning=FALSE}
...
* *The data presented in this report provides evidence that training on a dataset where all numeric predictors were standardized ONLY reduces the error for the Mulinomial Logistic Regression (multinom) classifier.*  This was a bit of a surprise, as it was expected that the Naive Bayes, LDA, and SVM classifiers would show improvement in their error rates.  It is possible that internally some of these classifiers are already doing the scaling on the training sets, in which case scaling as part of pre-processing upstream from training would be unnecessary. 

* *The Multinomial Logistic Regression (multinom) showed significant reduction in its test error, down 15% from .4 to .25 from Phase 0 to Phase 2. All other classifiers had no change in their error rates from Phase 0 to Phase 2.* 
