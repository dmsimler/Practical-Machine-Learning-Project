...
Finally, build other miscellaneous classifiers of interest:
...
CreateModelPerfAssessments <- function(modelPerfComparisons, comparisonLevelOrder)
{
  ...
  rownames(allPerfAssessments) <- NULL
...
# Define the model comparisons for Phase 1
ModelComparisonOrderedLevelsPhase1 <- c("rf - xgbTree", "rf - treebag", "rf - gbm",
                                  "rf - nb", "rf - lda", "rf - multinom", "rf - rpart", 
                                  ...
                                  "nb - gbm", "nb - lda", "nb - multinom", "nb - rpart",
                                  "lda - rf", "lda - xgbTree", "lda - treebag", 
                                  "lda - gbm", "lda - nb", "lda - multinom", "lda - rpart",  
                                  "rpart - rf", "rpart - xgbTree", "rpart - treebag", 
                                  "rpart - gbm", "rpart - nb",  "rpart - lda", "rpart - multinom",
                                  "multinom - rf", "multinom - xgbTree", "multinom - treebag",
                                  "multinom - gbm", "multinom - nb", "multinom - lda", "multinom - rpart")
...
pander(modelResultsPhase1$FinalModelSummaries, caption = "Table 1: Out-of-sample errors and final model parameters for the candidate classification models created above.  The lower the OOS Error, the more accurate is its classifier.")
...
grid.arrange(rfCompPlot, xgbCompPlot, bagCompPlot, gbmCompPlot, nbCompPlot, ldaCompPlot, rpartCompPlot,
             multinomCompPlot, nrow = 4, ncol = 2)
...
CreateModelTestPerfSummary <- function(modelTestPredicitons)
{
  # Create a dataframe with the error rate for the given model
  error <- 1 - (sum(modelTestPredicitons$PredictedClasse == modelTestPredicitons$ActualClasse) /
                nrow(modelTestPredicitons))
  modelTestPerfSummary <- data.frame(Model = modelTestPredicitons[1, "Model"], Error = error)
...
CreateTestPredictionDatasets <- function(modelList, testDataset)
{
   ...
   allModelsTestPerfsummary <- allModelsTestPerfsummary[order(allModelsTestPerfsummary$Error), ]
...
Now build the scaled linear classifiers:
...
Finally, build some scaled miscellaneous classifiers:
...
CreatePhasePerfCompDataset <- function(modelsOOSErrorsPhase1, modelsOOSErrorsPhase2, comparisonLevelOrder)
{
  # Get a list of all the different models in the phase 1 dataset.  It is assumed that the same set of models 
  # exist in phase 2.
  modelNames <- levels(modelsOOSErrorsPhase1$Model)

  allComparisons <- NULL
  for(comparisonIndex in 1:length(modelNames))
  {
    # Grab the out-of-sample error data for each model
    modelName <- modelNames[comparisonIndex]
    ...
    model1Errors <- model1Errors[order(model1Errors$FoldName), ]
    model2Errors <- model2Errors[order(model2Errors$FoldName), ]
    errorDiff = model2Errors$OOSError - model1Errors$OOSError
  ...
  return (allComparisons)
} # end CreatePhasePerfCompDataset
...
CreateScaledModelingResultsDatasets <- function(modelList, modelsOOSErrorsPhase1, modelComparisonOrderedLevels)
{
  ...
  modelsOOSErrorsPhase2 <- CreateModelsOOSErrorDataset(modelList)
  finalModelSummaries <- CreateModelTuningSummaries(modelList)
  perfComparisons <- CreatePhasePerfCompDataset(modelsOOSErrorsPhase1, modelsOOSErrorsPhase2,
...
ModelComparisonOrderedLevelsPhase2 <- c("rf2 - rf1", "xgbTree2 - xgbTree1", "treebag2 - treebag1", 
                                        "gbm2 - gbm1", "nb2 - nb1", "multinom2 - multinom1",
                                        "lda2 - lda1", "rpart2 - rpart1") 
...
panderOptions('table.alignment.default',"left")
pander(modelResultsPhase2$FinalModelSummaries, caption = "Table 3: Out-of-sample errors and final model parameters for the SCALED candidate classification models created for Phase 2.  The lower the OOS Error, the more accurate is its classifier.")
...
```{r, echo=FALSE, fig.path="figures/", fig.align='center', fig.cap='Figure 4: Distribution of the out-of-sample errors across 50 folds for the different SCALED models.  The lower (closer to zero) a distribution is, the more accurate is its classifier.', fig.height=5, warning=FALSE}
...
As in Phase 1, we first compare the scaled models with each other to see if there is any difference between them.  The boxplots of these differences are then shown below for each scaled model:

```{r, echo=FALSE, fig.path="figures/", fig.align='center', fig.cap='Figure 3: Distribution of the difference between out-of-sample errors across 50 folds for the different model pairwise comparisons', fig.height=6, fig.show='asis', warning=FALSE}
...
As in Phase 1, perform a t-test on each scaled model comparison's mean difference in out-of-sample error:
...
pander(modelResultsPhase2$PhasePerfAssessments[, -3], 
       caption = "Table 4: Model pair-wise t-test comparisons")
...
panderOptions('table.alignment.default',"left")
pander(scaledTestPredictionDatasets$ModelTestPerfSummary, caption = "Table 3: Error rate by model for predictions against a test dataset of 20 observations.")
